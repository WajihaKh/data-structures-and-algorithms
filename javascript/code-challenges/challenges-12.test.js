...
function transformToLis(obj){
  ...
  for (const [key, value] of Object.entries(obj)) {
    const listItems = `<li>${key}: ${value}</li>`;
    lis.push(listItems);
  }
  return lis;
}
...
const count = (target, input) => {
  ...
  const matchingElements = flattenedArray.filter((num) => num === target);
...
const totalSum = (input) => {
  return input.reduce((acc, arr) => acc + arr.reduce ((sum, num) => sum + num, 0), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => arr.filter(num => typeof num === 'number' && num % 5 === 0).map(num => Math.pow(2, num)));
};
