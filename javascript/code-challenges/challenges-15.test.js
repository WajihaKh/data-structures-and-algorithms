...
const toTitleCase = (arr) => {
  const titleCasedArray = arr.map((str) => {
...
let biggerThanLuke = (arr) => {
  ...
  const charactersWithGreaterMass = arr.filter(character => {
    if (character.name !== 'Luke Skywalker') {
      const characterMass = parseInt(character.mass);
      return characterMass > lukeMass;
    }
    return false;
  });


  const namesString = charactersWithGreaterMass.map(character => character.name).join(' - ');
